...
class ExactExpenseStrategy(ExpenseStrategy):
    def validate(self, no_of_users, amount_list, total_amount):
        return amount_list and (len(amount_list) == no_of_users) and sum(amount_list) == total_amount
    ...
    def create_expense(self, group, payment_made_by_user_id, total_amount, no_of_users, users_own_money_list,
        ...
        if self.validate(no_of_users, amount_list, total_amount):
            ...
            for i, user_own_money in enumerate(users_own_money_list):
                ...
                self.add_expense_owing_user(expense, user, amount_list[i])
            expense.participants.add(*money_owning_users)
            return expense
        else:
            raise Exception("Amount list does not match no. of users or total amount")
